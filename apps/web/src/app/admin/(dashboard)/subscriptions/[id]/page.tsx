...
const AdminSubscriptionEditPage: FC = () => {
  ...
  const update = trpc.admin.subscriptions.update.useMutation({
    onMutate: async (vars) => {
      ...
      const prevList = utils.admin.subscriptions.list.getData({ search: undefined, status: undefined, subscriptionTier: undefined, limit: 20, page: 1 })

      if (prevDetail) {
        utils.admin.subscriptions.byId.setData({ id: subscriptionId! }, { ...prevDetail, ...vars.patch })
      }

      if (prevList) {
        utils.admin.subscriptions.list.setData({ search: undefined, status: undefined, subscriptionTier: undefined, limit: 20, page: 1 }, {
          ...prevList,
          items: prevList.items.map(s => s.id === subscriptionId ? { ...s, ...vars.patch } : s)
        })
      }
    ...
    onError: (error, vars, ctx) => {
      ...
      if (ctx?.prevList) {
        utils.admin.subscriptions.list.setData({ search: undefined, status: undefined, subscriptionTier: undefined, limit: 20, page: 1 }, ctx.prevList)
      }
    },
    onSuccess: () => {
  ...
  if (error || !subscription) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-muted-foreground">{error?.message || "Abonnement introuvable"}</p>
      </div>
    )
  }
