...
import { UserDetailController } from '@/app/admin/(dashboard)/users/[id]/components/user-detail-controller'
...
const AdminUserEditPage: FC = () => {
  ...
  const { data: user, isLoading, error } = trpc.admin.users.detail.useQuery(
    { userId },
    {
      enabled: !!userId,
      retry: 1,
  ...
  const userData = useMemo(() => {
    ...
    return {
      ...
      email: user.email,
      role: (user.user_level === 'ambassadeur' ? 'admin' : 'user') as 'admin' | 'user',
      is_active: true,
  ...
  const update = trpc.admin.users.update.useMutation({
    onMutate: async (vars) => {
      ...
      if (prevDetail) {
        utils.admin.users.detail.setData({ userId }, { ...prevDetail, ...vars.patch })
      }

      if (prevList) {
        utils.admin.users.list.setData(undefined, {
          ...prevList,
          items: prevList.items.map(u => u.id === vars.userId ? { ...u, ...vars.patch } : u)
        })
      }
