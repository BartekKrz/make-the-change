...
const getDefaultProducerOptions = (
  t: (key: string) => string
): SelectOption[] => [
...
const ProductsPage: FC = () => {
  ...
  const [optimisticTags, removeOptimisticTag] = useOptimistic(
    selectedTags,
    (currentTags, tagToRemove: string) =>
  ...
  const { data: categories, isPending: isPendingCategories } =
    trpc.admin.categories.list.useQuery({ activeOnly: true });
  const { data: tagsData, isPending: isPendingTags } =
    trpc.admin.products.tags.useQuery({ activeOnly: true, withStats: true });
  const {
    data: productsData,
    isLoading,
    isFetching,
  ...
  const producerOptions = useMemo(
    (): SelectOption[] =>
      isPendingProducers || !producers
        ? getDefaultProducerOptions(t)
        : createSelectOptions(
            producers,
          ...
          <div className="flex items-center gap-4">
            <div className="flex flex-1 flex-wrap items-center gap-3">
              ...
              <SimpleSelect
                ...
                disabled={
                  isPendingCategories || !categories || isPendingFilters
                }
                onValueChange={value =>
                  handleFilterChange(() =>
      ...
      <AdminPageLayout.Content>
        {isError ? (
          <EmptyState
            ...
            description={
              error?.message || t('admin.products.error.loading_description')
            }
          />
        ) : (
          <DataList
      ...
      >
        <Filters>
          ...
          <Filters.Selection
            ...
            onSelectionChange={id =>
              handleFilterChange(() => setSelectedProducerId(id))
            }
          />
          ...
          <Filters.Selection
            ...
            onSelectionChange={id =>
              handleFilterChange(() => setSelectedCategoryId(id))
            }
          />
