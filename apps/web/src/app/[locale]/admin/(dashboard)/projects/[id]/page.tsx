...
const AdminProjectEditPage: FC = () => {
  ...
  const { data: project, isLoading, error } = trpc.admin.projects.byId.useQuery(
    { id: projectId! },
    {
      enabled: !!projectId,
      retry: 1,
  ...
  const update = trpc.admin.projects.update.useMutation({
    onMutate: async (vars) => {
      ...
      if (prevDetail) {
        utils.admin.projects.byId.setData({ id: projectId! }, { ...prevDetail, ...vars.patch })
      }

      if (prevList) {
        utils.admin.projects.list.setData(undefined, {
          ...prevList,
          items: prevList.items.map(p => p.id === projectId ? { ...p, ...vars.patch } : p)
        })
      }
