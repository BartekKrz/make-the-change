...
const AdminSubscriptionsPage: FC = () => {
  ...
  const [status, setStatus] = useState<
    'active' | 'cancelled' | 'suspended' | 'past_due' | undefined
  >();
  const [subscriptionTier, setSubscriptionTier] = useState<
    'ambassadeur_standard' | 'ambassadeur_premium' | undefined
  >();
  const utils = trpc.useUtils();

  const handleStatusChange = (value: string) => {
    setStatus(
      value as 'active' | 'cancelled' | 'suspended' | 'past_due' | undefined
    );
  };

  const handleTierChange = (value: string) => {
    setSubscriptionTier(
      value as 'ambassadeur_standard' | 'ambassadeur_premium' | undefined
    );
  };
  ...
  const updateSubscription = trpc.admin.subscriptions.update.useMutation({
    onMutate: async vars => {
      await utils.admin.subscriptions.list.cancel();
      const prev = utils.admin.subscriptions.list.getData({
        search: search || undefined,
        status: status || undefined,
        subscriptionTier: subscriptionTier || undefined,
        limit: 20,
                ...
                <DataCard.Title>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">
                      {s.users?.user_profiles?.first_name}{' '}
                      {s.users?.user_profiles?.last_name}
                    </span>
