-- =====================================================
-- MIGRATION SQL COMPLÈTE - SYSTÈME DUAL BILLING
-- Make the CHANGE - Système d'Abonnements Innovant
-- =====================================================
-- 
-- Ce script crée toutes les tables et structures nécessaires
-- pour le système dual billing (mensuel/annuel) avec économie de points
--
-- ORDRE D'EXÉCUTION :
-- 1. Extensions et Types
-- 2. Tables principales 
-- 3. Tables d'analytics
...
-- Extension pour UUID automatiques (probablement déjà activée)
...
-- =====================================================
-- 2. MISE À JOUR TABLE SUBSCRIPTIONS EXISTANTE
-- =====================================================

-- Étendre la table subscriptions existante avec les colonnes dual billing
ALTER TABLE subscriptions 
ADD COLUMN IF NOT EXISTS plan_type subscription_plan_type DEFAULT 'explorer_free',
ADD COLUMN IF NOT EXISTS billing_frequency billing_frequency DEFAULT 'monthly',
...
ADD COLUMN IF NOT EXISTS annual_points INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS bonus_percentage DECIMAL(5,2) DEFAULT 0.00,
ADD COLUMN IF NOT EXISTS current_period_start TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS current_period_end TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS next_billing_date TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS trial_end TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS converted_from billing_frequency,
ADD COLUMN IF NOT EXISTS conversion_date TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS conversion_incentive JSONB DEFAULT '{}',
...
DO $$ BEGIN
    ALTER TABLE subscriptions ADD CONSTRAINT check_billing_consistency 
    CHECK (
        (billing_frequency = 'monthly' AND monthly_price IS NOT NULL) OR
        (billing_frequency = 'annual' AND annual_price IS NOT NULL)
    );
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
-- =====================================================
-- 3. ÉTENDRE TABLE MONTHLY_ALLOCATIONS
-- =====================================================
...
ADD COLUMN IF NOT EXISTS allocated_investments INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS allocated_products INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS unused_points INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS rollover_from_previous INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS rollover_to_next INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS points_expired INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS expiry_date TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS allocation_preferences JSONB DEFAULT '{"investments": 70, "products": 30}',
ADD COLUMN IF NOT EXISTS metadata JSONB DEFAULT '{}';
...
BEGIN
    ...
    LOOP
        ...
        INSERT INTO points_transactions (
            user_id, amount, transaction_type, description, related_entity_type, related_entity_id
        ) VALUES (
...
BEGIN
    ...
    IF NEW.amount > 0 AND NEW.transaction_type IN ('subscription', 'bonus', 'referral') THEN
...
-- Politiques pour business_metrics (admin seulement)
CREATE POLICY "Only admins can access business metrics" ON business_metrics
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM users 
            WHERE id = (SELECT auth.uid()) 
...
CREATE POLICY "Admins can view all points expiry" ON points_expiry_schedule
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM users 
            WHERE id = (SELECT auth.uid()) 
...
-- Politiques pour subscription_cohorts (admin seulement)
CREATE POLICY "Only admins can access cohort data" ON subscription_cohorts
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM users 
            WHERE id = (SELECT auth.uid()) 
...
-- Insérer des plans de test si les utilisateurs existent
DO $$
DECLARE
    test_user_id UUID;
...
BEGIN
    ...
    IF test_user_id IS NOT NULL THEN
        -- Créer un abonnement mensuel de test
        INSERT INTO subscriptions (
            id, user_id, plan_type, billing_frequency, 
            monthly_price, annual_price, monthly_points, annual_points,
            status, current_period_start, current_period_end, next_billing_date,
            stripe_subscription_id, stripe_customer_id
        ) VALUES (
            uuid_generate_v4(), test_user_id, 'ambassador_standard', 'monthly',
            18.00, 180.00, 252, 2520,
            'active', NOW(), NOW() + INTERVAL '1 month', NOW() + INTERVAL '1 month',
            'sub_test_monthly_001', 'cus_test_001'
        ) RETURNING id INTO test_subscription_id;
        
        -- Créer un historique de facturation de test
        INSERT INTO subscription_billing_history (
            subscription_id, user_id, billing_period_start, billing_period_end,
            amount, plan_type, billing_frequency, points_allocated,
            stripe_invoice_id, payment_status, paid_at
        ) VALUES (
            test_subscription_id, test_user_id, NOW() - INTERVAL '1 month', NOW(),
            18.00, 'ambassador_standard', 'monthly', 252,
            'in_test_001', 'paid', NOW() - INTERVAL '1 month'
        );
        
        -- Créer une allocation mensuelle de test
        INSERT INTO monthly_allocations (
            user_id, subscription_id, total_points, billing_period,
            allocated_investments, allocated_products,
            allocation_preferences
        ) VALUES (
            test_user_id, test_subscription_id, 252, TO_CHAR(NOW(), 'YYYY-MM'),
            176, 76, -- 70% investissements, 30% produits
            '{"investments": 70, "products": 30}'
        );
    END IF;
END $$;
...
BEGIN
    RAISE NOTICE '==============================================';
    RAISE NOTICE 'MIGRATION DUAL BILLING TERMINÉE AVEC SUCCÈS';
    RAISE NOTICE '==============================================';
