...
│ ├─────┼─────────────┼─────────┼─────────┼─────────┼─────────┤ │
│ │ T1  │ Claire N.   │ Earn    │ +264    │ Active  │ [👁️] [✏️] │ │
...
```typescript
// Formule de base
const calculatePoints = (investmentAmount: number, userLevel: string, loyaltyMultiplier: number) => {
  
  
  return Math.floor(investmentAmount * baseRate[tier] * loyaltyMultiplier);
};

// Expiry par défaut
const defaultExpiryDate = (earnedDate: Date) => {
  const expiry = new Date(earnedDate);
  expiry.setMonth(expiry.getMonth() + 18); // 18 mois
  return expiry;
};
```
...
const pointsActions = {
  ...
  viewInvestmentPoints: `/admin/investments/${investmentId}?tab=points`,
